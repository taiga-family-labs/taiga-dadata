...
Publishing is handled by `.github/workflows/publish.yml` when a GitHub Release is published. Use a semver tag prefixed with `v`, for example `v0.1.0`. The workflow derives the package version from the release tag, runs type checking, builds the library and publishes `dist/taiga-dadata` with npm provenance.
